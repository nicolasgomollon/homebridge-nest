  ...
  switch (value) {
    ...
    case Characteristic.TargetHeatingCoolingState.HEAT:
      if (this.device.can_heat === true) {
        ...
        this.log("Device does not support heating, cannot set mode to " + value);
        return;
      }
    case Characteristic.TargetHeatingCoolingState.COOL:
      if (this.device.can_cool === true) {
        ...
        this.log("Device does not support cooling, cannot set mode to " + value);
        return;
      }
...
NestThermostatAccessory.prototype.getCurrentTemperature = function () {
  if (this.device.temperature_scale == "F") {
    return 1.010 * this.device.ambient_temperature_c;
  }
...
NestThermostatAccessory.prototype.getTargetTemperature = function () {
  switch (this.device.hvac_mode) {
    ...
    default:
      if (this.device.temperature_scale == "F") {
        return 1.015 * this.device.target_temperature_c;
      }
...
NestThermostatAccessory.prototype.getCoolingThresholdTemperature = function () {
  switch (this.device.hvac_mode) {
    case "heat-cool":
      if (this.device.temperature_scale == "F") {
        return 1.010 * this.device.target_temperature_high_c;
      }
      return this.device.target_temperature_high_c;
    case "eco":
      if (this.device.temperature_scale == "F") {
        return 1.010 * this.device.eco_temperature_high_c;
      }
...
NestThermostatAccessory.prototype.getHeatingThresholdTemperature = function () {
  switch (this.device.hvac_mode) {
    case "heat-cool":
      if (this.device.temperature_scale == "F") {
        return 1.015 * this.device.target_temperature_low_c;
      }
      return this.device.target_temperature_low_c;
    case "eco":
      if (this.device.temperature_scale == "F") {
        return 1.010 * this.device.eco_temperature_low_c;
      }
...
NestThermostatAccessory.prototype.setTargetTemperatureDebounced = debounce(function (temperature, callback) {
  switch (this.device.hvac_mode) {
    ...
    case "off":
      ...
      return;
    default:
      const scaledTemperature = Math.round((temperature * 2) / 2);
      return Promise.resolve()
        .then(this.updateDevicePropertyAsync.bind(this, "target_temperature_c", scaledTemperature, "target temperature"))
        .asCallback(callback);
  }
}, 5000);
...
NestThermostatAccessory.prototype.setCoolingThresholdTemperatureDebounced = debounce(function (temperature, callback) {
  switch (this.device.hvac_mode) {
    case "heat-cool":
      const scaledTemperature = Math.round((temperature * 2) / 2);
      return Promise.resolve()
        .then(this.updateDevicePropertyAsync.bind(this, "target_temperature_high_c", scaledTemperature, "cooling threshold temperature"))
        .asCallback(callback);
    default:
...
NestThermostatAccessory.prototype.setHeatingThresholdTemperatureDebounced = debounce(function (temperature, callback) {
  switch (this.device.hvac_mode) {
    case "heat-cool":
      const scaledTemperature = Math.round((temperature * 2) / 2);
      return Promise.resolve()
        .then(this.updateDevicePropertyAsync.bind(this, "target_temperature_low_c", scaledTemperature, "heating threshold temperature"))
        .asCallback(callback);
    default:
