...
function usesCelsius() {
  return this.device.temperature_scale === "C";
}

function usesFahrenheit() {
  return this.device.temperature_scale === "F";
}
  ...
  switch (value) {
    ...
    case Characteristic.TargetHeatingCoolingState.AUTO:
      if (this.device.can_heat === true && this.device.can_cool === true) {
        ...
        this.log("Device does not support heating or cooling, cannot set mode to " + value);
        return;
      }
    default:
      state = "off";
...
NestThermostatAccessory.prototype.setTargetTemperatureDebounced = debounce(function (temperature, callback) {
  const target = usesFahrenheit() ? "target_temperature_f" : "target_temperature_c";
  switch (this.device.hvac_mode) {
    case "eco":
    case "heat-cool":
    case "off":
...
NestThermostatAccessory.prototype.setCoolingThresholdTemperatureDebounced = debounce(function (temperature, callback) {
  const target = usesFahrenheit() ? "target_temperature_high_f" : "target_temperature_high_c";
  switch (this.device.hvac_mode) {
    case "heat-cool":
      return Promise.resolve()
...
NestThermostatAccessory.prototype.setHeatingThresholdTemperatureDebounced = debounce(function (temperature, callback) {
  const target = usesFahrenheit() ? "target_temperature_low_f" : "target_temperature_low_c";
  switch (this.device.hvac_mode) {
    case "heat-cool":
      return Promise.resolve()
